...
def shortest_path(source, target):
    ...
    #keep a count of states explored
    ...
    while True:
        ...
        #else remove a node from the frontier and add to explored set

        node = queue.remove()
        explored.add(node)
        exploredCount += 1

        #find the neighbors of the actor in node
        ...
        for movie, actor in neighbors:
            if actor not in explored and not queue.contains_state(actor):
                ...
                #check if the child is the required state, if so return a path
